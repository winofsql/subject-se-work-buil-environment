...
If not objFSO.FolderExists(target0) Then
	...
	Wscript.Echo "git.zip を copy しています。しばらくお待ちください"

	Set sourceFile = objFSO.GetFile("G:\共有ドライブ\SE-WORK-DOWNLOAD\software\git.zip")
	sourceFile.Copy target2 & "\git.zip", True

	Wscript.Echo "git.zip の copy が終了しました"

	Wscript.Echo "git.zip を 解凍しています。しばらくお待ちください"

	objShell.Run Chr(34) & SevenZipPath & Chr(34) & " x -o" & target0 & " " & target0 & ".zip", 0, True

	Wscript.Echo "git.zip の 解凍が終了しました"

End If

MsgBox("git 設定を終了しました。")
