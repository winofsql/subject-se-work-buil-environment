...
If not objFSO.FolderExists(target2 ) Then
	Wscript.Echo "WinOfSql105.zip をGドライブからコピーしています......"

	Set sourceFile = objFSO.GetFile("G:\共有ドライブ\SE-WORK-DOWNLOAD\database\WinOfSql105.zip")
	sourceFile.Copy target1 & "\WinOfSql105.zip", True

	Wscript.Echo "WinOfSql105.zip をGドライブからコピーしました"

	Wscript.Echo "WinOfSql105.zip を解凍しています......"
	
	objShell.Run Chr(34) & SevenZipPath & Chr(34) & " x -o" & target1 & "\WinOfSql105 " & target1 & "\WinOfSql105.zip", 0, True

	Wscript.Echo "WinOfSql105.zip を解凍しました"
	Wscript.Echo "WinOfSql105.zip を削除しています......"

	objFSO.DeleteFile target1 & "\WinOfSql105.zip"

	Wscript.Echo "WinOfSql105.zip を削除しました"

End If

work = objShell.ExpandEnvironmentStrings("C:\Users\%USERNAME%\Desktop")

set oShellLink = objShell.CreateShortcut(work & "\WinOfSql.lnk")
oShellLink.TargetPath = target1 & "\WinOfSql105\WinOfSql.exe"
'oShellLink.Arguments = ""
oShellLink.WindowStyle = 1
oShellLink.IconLocation = target1 & "\WinOfSql105\WinOfSql.exe"
oShellLink.WorkingDirectory = target1 & "\WinOfSql105"
oShellLink.Save

MsgBox("SQLの窓(WinOfSql) 設定を終了しました。")
